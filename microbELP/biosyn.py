...
from microbELP import load_saved_dictionary
...
class BioSyn(object):
    ...
    def load_sparse_encoder(self, model_name_or_path):
        ...
        if not os.path.isfile(sparse_encoder_path):
            ...
            sparse_encoder_path = hf_hub_download(repo_id=model_name_or_path, filename="sparse_encoder.pk")
...
def cache_or_load_dictionary(biosyn, model_name_or_path):

    dictionary = load_saved_dictionary()
...
def cache_or_load_dictionary_ontology(biosyn, model_name_or_path, dictionary_path):
    ...
    if save:
        with open('./microbiome_biosyn_normalisation_output.json', 'w', encoding='utf-8') as f:
            ...
            for i, entry in enumerate(final_output):
                ...
                candidates_lines = [f'      {json.dumps(c, ensure_ascii=False)}' for c in entry["candidates"]]
