...
from src.rank_counts import rank_hierarchy

def generate_master_positions(combined_data, output_file=None):
    # joram's approach
...
def load_master_positions(file_path):
...
def plot_phylogenetic_tree_with_master_positions(
    ...
    abundance_threshold=None, max_textbox_labels=10):
    ...
    ensure_parent_relationships(node_positions, tax_parent_map, tax_children_map, filtered_rank_abundances_dict)
    ...
    def get_quartile_colour(abundance):
        if abundance <= q1:
    ...
    def boxes_overlap(box1, box2, buffer=0.05):
    ...
    create_abundance_legend(fig, ax, edge_cmap, bottom_position + 0.1)
...
def ensure_parent_relationships(node_positions, tax_parent_map, tax_children_map, filtered_rank_abundances_dict):
...
def create_abundance_legend(fig, ax, edge_cmap, bottom_position):
...
def compare_microbiomes_with_master_positions(microbiome_data_list, labels=None, output_dir=None, master_positions=None):
    ...
    if master_positions is None:
        ...
        master_positions = generate_master_positions(microbiome_data_list[0])
    ...
    for i, (data, label) in enumerate(zip(microbiome_data_list, labels)):
        ...
        fig, ax = plot_phylogenetic_tree_with_master_positions(
            filtered_rank_abundances_dict, tax_parent_map, tax_children_map, 
            tax_2_rank_string_map, tax_2_normalised_rank_value_map, master_positions,
            surface_text=label, output_file=output_file
        )
