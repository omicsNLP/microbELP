...
def randsample(population, k, replace=True, weights=None):
...
def empirical_sampling_comparison(background_rank_dict, domain_rank_dict, n_samp=1000, fdr_method='storey'):
    ...
    for rank in domain_rank_dict:
        ...
        if fdr_method.lower() == 'storey':
            q_values = storey_tibshirani_fdr(p_values)
...
def transform_results_to_dataframe(results, taxonomy_data):
